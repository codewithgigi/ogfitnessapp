import * as convertIngredientMeasurement from "./convert";
...
function getUnit(input) {
  for (const unit of Object.keys(units)) {
    for (const shorthand of units[unit]) {
      if (input.toLowerCase() === shorthand) {
        return [unit, input];
      }
    }
  }
...
function parse(recipeString) {
  ...
  const [unit, originalUnit] = getUnit(restOfIngredient.split(" ")[0]);
...
function scale(ingredients, serving, actualServing) {
...
function goodEnoughCombineTwoIngredients(existingIngredients, ingredient) {
  const unit = existingIngredients?.unit ?? " ";
  if (!Qty.parse(unit)) {
    if (existingIngredients?.unit === ingredient?.unit) {
      ...
      return Object.assign(
        {},
        { ...existingIngredients, quantity: `${quantity}` },
      );
    }
  } else {
    ...
    try {
      ...
      let unit = combinedIng.units();
      if (unit === "cu") unit = "cup";
      if (unit === "tb") unit = "tablespoon";
 ...
 */
function goodEnoughCombine(ingredientArray) {
  const combinedIngredients = ingredientArray.reduce((acc, ingredient) => {
    ...
    try {
      ...
      e.message = `${e.message}. Error while creating qty ${JSON.stringify(
        ingredient,
      )}`;
      //return acc;
    }
    ...
    let key;
    if (!ingQty) {
      key = `${ingredient.ingredient}-${normalizeUnit(ingredient.unit)}`;
    } else key = `${ingredient.ingredient}-${ingQty.kind()}`; // when combining different units, remove this from the key and just use the name
    const existingIngredient = acc[key];
  ...
  const combinedList = Object.keys(combinedIngredients)
    .reduce((acc, key) => {
      const ingredient = combinedIngredients[key];
      return acc.concat(ingredient);
    }, [])
...
export { goodEnoughCombine, parse, combine, scale };
